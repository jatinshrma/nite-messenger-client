...
const UserProfile = () => {
	...
	const UpdateHandler = async e => {
		e.preventDefault()
		!imageResponse.file && updateProfile(updateData, user)
		file && deletePicture(profile?.dp_id) && (await uploadPicture(file))
			...
			<div className="card mb-3 bg-dark">
				<div className="row g-0">
					<div className="col-md-4" id="profile_page_dp_div">
						{editContent && (
							<form name="fileForm" onSubmit={uploadPicture} encType="multipart/form-data">
								<input type="file" id="dp_update_input" onChange={onDPChange} className="d-none" />
							</form>
						)}
						<label htmlFor="dp_update_input">
							{profile && (
								<img
									src={fileBase64String ? fileBase64String : `${USER_DP}/${profile?.dp_id}`}
									id="profile_page_dp"
									alt="..."
								/>
							)}
						...
						<div className="card-body g-0 card p-2 bg-dark">
								...
								{editContent ? (
									<>
										<textarea
											...
											onChange={onchange}
											className="bio-textarea form-control"
											maxLength="150"
											style={{ outline: "none" }}
										/>
										{updateData?.bio && (
											<span className="list-value-bio mx-2" id="bio-length">
												{updateData?.bio.length}/150
											</span>
										)}
									</>
								) : (
									<p className="list-value-bio card-text">{updateData?.bio || profile?.bio}</p>
								)}
							</div>
							<ul className="list-group list-group-flush">
								<li className="list-group-item bg-dark text-white d-flex">
									...
									{!editContent && (
										<div className="list-values">{updateData?.name ? updateData?.name : profile?.name}</div>
									)}
									{editContent && (
								...
								<li className="list-group-item bg-dark text-white d-flex">
									...
									{editContent ? (
										...
										<div className="list-values">{updateData?.email ? updateData?.email : profile?.email}</div>
									)}
								</li>
							</ul>
						</div>
						{editContent ? (
							<button onClick={UpdateHandler} className="btn m-2 mx-4">
								Save
							</button>
						) : (
							<button onClick={() => setEditContent(true)} className="btn m-2 mx-4">
								Edit
							</button>
						)}
					</div>
